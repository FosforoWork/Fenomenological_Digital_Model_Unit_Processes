...
3. Humedad de torta para balance integrado: 40 % bh (supuesto aprobado).
...
$$
m_w = 1.2\frac{\text{kg agua}}{\text{kg solido}}\times3.50 = 4.20\ \text{kg}
$$

### 4.3 Concentracion de solidos

$$
C_{s,kg/kg} = \frac{m_s}{m_s+m_w}=\frac{3.50}{7.70}=0.4545\ \text{kg solido/kg mezcla}
$$
...
$$
C_{s}\approx454.5\ \text{kg/m}^3\ (\approx454\ \text{kg/m}^3\ de la tabla)
$$
...
Supuesto aprobado: humedad de torta = 40 % bh.

Para $m_s=3.50$ kg en la torta:

$$
m_{torta,hum} = \frac{m_s}{1-0.40} = 5.8333\ \text{kg}
$$

$$
m_{agua,torta}=5.8333-3.50=2.3333\ \text{kg}
$$

Filtrado de la base integrada (balance de lote):

$$
m_{filtrado}=7.70-5.8333=1.8667\ \text{kg}
$$
...
Con base de $m_s=3.50$ kg de solido seco:

$$
m_{producto}=m_s(1+X_f)=3.50(1+0.0869565)=3.8043\ \text{kg}
$$

### 6.3 Agua evaporada

**Base de curva experimental** ($X_i=1.97$):

$$
m_{evap,curva}=m_s(X_i-X_f)=3.50(1.97-0.0869565)=6.5907\ \text{kg}
$$

**Base integrada de linea** (consistente con torta 40 % bh):

$$
m_{evap,int}=m_{agua,torta}-m_{agua,final}=2.3333-0.3043=2.0290\ \text{kg}
$$
...
Tiempo equivalente desde una torta de 40 % bh ($X=0.6667$) sobre la curva:

$$
t(X=0.6667)\approx74.8148\ \text{min},\quad
\Delta t_{0.6667\to0.08696}\approx52.0773\ \text{min}
$$
...
**Base integrada de linea:**

$$
Q_{min,int}=m_{evap,int}\lambda=2.0290\times2257=4579.4203\ \text{kJ}
$$

$$
Q_{min,int}=1.2721\ \text{kWh}
$$

**Base de curva experimental:**

$$
Q_{min,curva}=6.5907\times2257=14875.1020\ \text{kJ}=4.1320\ \text{kWh}
$$
...
| Filtracion | 7.7000 | 5.8333 (torta) + 1.8667 (filtrado) = 7.7000 | 0.0000 % |
| Secado | 5.8333 | 3.8043 (producto) + 2.0290 (evaporada) = 5.8333 | 0.0000 % |
...
$$
m_{out}=6.5\ (\text{rechazos tamizado})+1.8667\ (\text{filtrado})+3.8043\ (\text{producto})+2.0290\ (\text{agua evaporada})
$$
...
| Suspension: masa de solido seco | 3.50 kg |
| Suspension: masa de agua anadida | 4.20 kg |
| Suspension: concentracion de solidos | 0.4545 kg/kg (45.45 %) |
| Filtracion: volumen de filtrado | 20.0 L (serie experimental) |
| Filtracion: masa de torta humeda | 5.8333 kg |
| Filtracion: humedad de torta | 40 % bh (supuesto aprobado) |
...
| Secado: masa final producto | 3.8043 kg |
| Secado: agua evaporada | 2.0290 kg (integrado) y 6.5907 kg (base curva) |
| Secado: tiempo para 8 % bh | 126.8921 min (extrapolado) |
| Secado: energia de secado | 1.2721 kWh (integrado, minima teorica) |
