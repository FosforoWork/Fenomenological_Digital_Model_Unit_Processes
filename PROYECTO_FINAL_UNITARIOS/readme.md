...
# Blueprint de Ingeniería: Gemelo Digital AJAX V1
**Estado:** Fase de Migración y Optimización de Alta Fidelidad
**Responsable:** Jules (Unidad de Desarrollo e Ingeniería de Procesos)
**Framework Objetivo:** React Stack Industrial

A.J.A.X.: Autonomous Juxtaposition for Analytic X-linkage
(Yuxtaposición Autónoma para Vinculación Analítica) – El nombre definitivo para un gemelo digital.  
---

## 1. Re-Arquitectura del Núcleo (Engine)
*Transición de interfaz estática a motor reactivo con sincronización de estado.*

- [ ] **Configuración de Boilerplate Industrial:** Inicializar proyecto con `Vite + React + TypeScript`. El tipado estricto es obligatorio para evitar errores de redondeo en variables de masa y energía.
- [ ] **State Management (Single Source of Truth):** Implementar `Zustand` para manejar el estado global del sistema. Las variables de control ($pH$, $T^\circ$, Caudal) deben ser accesibles por cualquier nodo del gemelo sin prop-drilling.
- [ ] **Capa de Abstracción de Datos:** Crear hooks personalizados (e.g., `useProcessVariables`) que simulen el polling de sensores mediante WebSockets o Server-Sent Events.

## 2. UI/UX: High Performance HMI (ISA-101 Standard)
*Optimización de la carga cognitiva y estética de instrumentación profesional.*

- [ ] **Paleta de Colores "Dark Mode" Científico:** * Fondo: `#020617` (Deep Navy) o `#0f172a` (Slate).
    * Superficies: `#1e293b` con bordes sutiles.
    * Texto: `#f8fafc` (Slate 50) para legibilidad extrema.
- [ ] **Librería de Componentes:** Instalar `Shadcn/ui` y `Tailwind CSS`. Configurar componentes de "Glassmorphism" sutil para dar la profundidad visual de un gemelo digital.
- [ ] **Tipografía de Grado de Ingeniería:** Implementar `Inter` para la interfaz general y `JetBrains Mono` o `Roboto Mono` para todos los valores numéricos dinámicos (evita el parpadeo de layout cuando cambian los dígitos).

## 3. Visualización de Procesos y Telemetría
*Transformación de barras estáticas en indicadores de tendencia e inercia.*

- [ ] **Implementación de Micro-Tendencias (Sparklines):** Sustituir los sliders actuales por tarjetas que incluyan un gráfico de área (usando `Recharts` o `Tremor`) que muestre los últimos 60 segundos de la variable.
- [ ] **Capa de Visualización de Fluidogramas:** Migrar el "Fluidograma Integrado" a un componente SVG reactivo donde las líneas de flujo cambien de color o grosor según el caudal real del sistema.
- [ ] **Modo de Sensibilidad Avanzado:** En el modo exploratorio, implementar una función de "Ghosting" donde se vea la variable actual vs. la variable proyectada por el modelo matemático del Ph.D.

## 4. Lógica de Gemelo Digital (Digital Twin Logic)
*Añadiendo inteligencia y predictibilidad al sistema.*

- [ ] **Motor de Cálculo en Segundo Plano:** Implementar `Web Workers` para procesar los algoritmos de balance de materia y energía sin bloquear el hilo principal de la UI.
- [ ] **Semaforización por Desviación Estándar:** La lógica de colores no debe ser fija. Implementar una función que cambie a amarillo/rojo basada en la desviación respecto al Setpoint (SP) y no solo en rangos documentales.
- [ ] **Simulador de Inercia Térmica:** Programar que la "Temperatura de agua" no cambie instantáneamente, sino que siga una curva de respuesta de primer orden ($1 - e^{-t/\tau}$) para emular la física real.

---

## Notas de Implementación para Jules:
> "Jules, recuerda que el **AJAX V1** no es solo una cara bonita; es una representación matemática de un activo físico. Cada interacción del usuario debe pasar por el motor de validación antes de verse reflejada en el tablero. El objetivo es que el operador sienta que está tocando la tubería, no un sitio web."